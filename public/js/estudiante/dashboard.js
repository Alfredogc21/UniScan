...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const transitionDelay = 300; // ms, debe coincidir con la duración de la transición CSS    // Función para mostrar/ocultar el overlay
    ...
    function handleSidebarToggle(isMobile = false) {
        ...
        if (window.innerWidth <= 991 || isMobile) {
            if (sidebar.classList.contains('dashboard__sidebar--active')) {
                ...
                sidebarOverlay.classList.add('sidebar-overlay--active');
                document.body.style.overflow = 'hidden'; // Prevenir scroll del body cuando sidebar está abierto
            }
        } 
        // En desktop
        else {
    ...
    function initializeSidebarState() {
        ...
        if (window.innerWidth <= 991) {
            ...
            sidebar.classList.remove('dashboard__sidebar--active');
            sidebarOverlay.classList.remove('sidebar-overlay--active');
            // Asegurarse que el sidebar está posicionado correctamente en móviles
